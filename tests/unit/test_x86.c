...
static void test_x86_functioncall(void)
{
    uc_engine *uc;
    uc_hook hook;
    char code[] = "\x48\xc7\xc5\x00\x20\x00\x00\x55"; // IN eax, 0x10

    INSN_IN_RESULT result;

    uc_common_setup(&uc, UC_ARCH_X86, UC_MODE_64, code, sizeof(code) - 1);
    uint32_t r_eax = 199;
    //int64_t code_gen_prologue_ptr = 0x100;//uc->tcg_ctx->code_gen_prologue;
    //OK(uc_reg_write(uc, UC_X86_REG_RBP,code_gen_prologue_ptr));
